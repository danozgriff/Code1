...
def Notify(rundt):
    ...
    #closecnt = scraperwiki.sqlite.execute("select count(*) from Trades where position = 'Closing'")
      ...
      ranklist = scraperwiki.sqlite.execute("select * from ((select * from (select * from from Company_Performance order by StdDev_Rank limit 50)) intersect (select * from (select * from Company_Performance order by SignalAccuracy_Rank limit 50))) order by Overall_Rank")   
